...
export default function SemanticSearchPage() {
 ...
 const inputCls = "w-full bg-secondary-surface border border-border/[0.08] text-text-primary rounded-xl px-4 py-2.5 text-sm placeholder-muted focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/30 transition-all";
 const selectCls = "bg-secondary-surface border border-border/[0.08] text-primary text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500/30";
 const labelCls = "block text-[11px] text-muted uppercase tracking-widest mb-1 font-semibold";

 if (settingsLoading) {
 return (
 <AppLayout>
 <main className="min-h-screen p-6">
 <div className="rounded-2xl border border-border/[0.06] bg-secondary-surface p-12 text-center">
 <h3 className="text-lg font-semibold text-text-primary">Loading AI Search</h3>
 </div>
 </main>
 </AppLayout>
 );
 ...
 <main className="min-h-screen p-6">
 <div className="rounded-2xl border border-border/[0.06] bg-secondary-surface p-12 text-center">
 <h3 className="text-lg font-semibold text-text-primary">AI Search Disabled</h3>
 ...
 {/* Navigation Tabs */}
 <div className="flex border-b border-border/[0.06] mb-6 overflow-x-auto gap-1">
 {[
 ...
 <div className="mb-6">
 {searchTab === "standard" && (
 <form onSubmit={handleStandardSearch} className="bg-white/[0.03] p-6 rounded-2xl border border-border/[0.06]">
 ...
 type="submit"
 disabled={loading}
 className="bg-blue-600 hover:bg-blue-500 disabled:opacity-70 disabled:cursor-not-allowed text-white font-medium px-6 py-2.5 rounded-xl transition-all flex items-center justify-center min-w-[120px]"
 >
 {loading ? (
 ...
 {searchTab === "advanced" && (
 <form onSubmit={handleAdvancedSearch} className="bg-white/[0.03] p-6 rounded-2xl border border-border/[0.06] space-y-4">
 ...
 type="submit"
 disabled={loading}
 className="bg-blue-600 hover:bg-blue-500 disabled:opacity-70 disabled:cursor-not-allowed text-white font-medium px-6 py-2.5 rounded-xl transition-all flex items-center justify-center min-w-[200px]"
 >
 {loading ? (
 <>
 ...
 {searchTab === "jd" && (
 <form onSubmit={handleJdSearch} className="bg-white/[0.03] p-6 rounded-2xl border border-border/[0.06] space-y-4">
 <div>
 ...
 className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium px-6 py-2.5 rounded-xl transition-all flex items-center gap-2 min-w-[240px] justify-center"
 >
 {loading ? (
 <>
 ...
 {searchTab === "position" && (
 <div className="bg-white/[0.03] p-6 rounded-2xl border border-border/[0.06] space-y-4">
 <div>
 <div className="flex items-center justify-between mb-1">
 <label className={`${labelCls} !mb-0`}>Select Active Job Position</label>
 {loading && searchTab === "position" && (
 ...
 <div className="pt-4 border-t border-border/[0.06] animate-in fade-in duration-200">
 ...
 Showing <span className="text-text-secondary font-medium">{filteredAndSortedCandidates.length}</span>
 {totalCount > candidates.length && (
 <> of <span className="text-text-secondary font-medium">{totalCount}</span> total</>
 )} candidates
 </p>
 </div>
 )}
 ...
 {candidates.length > 0 && (
 <div className="bg-white/[0.03] p-4 rounded-xl border border-border/[0.06] flex flex-wrap gap-4 items-center justify-between">
 <div className="flex flex-wrap gap-4 items-center">
 {/* Location filter */}
 <div className="flex items-center gap-2">
 <MapPin className="h-4 w-4 text-muted" />
 <span className="text-[11px] text-muted uppercase tracking-widest">Location:</span>
 <select
 ...
 <span className="text-[11px] text-muted uppercase tracking-widest">Experience:</span>
 <select
 ...
 <span className="text-[11px] text-muted uppercase tracking-widest">Sort:</span>
 <select
 ...
 className="bg-secondary-surface border border-border/[0.08] hover:bg-secondary-surface text-secondary rounded-lg px-2.5 py-1.5 transition-all text-xs font-semibold uppercase"
 >
 {filters.sortOrder}
 </button>
 </div>
 ...
 <div className="w-full rounded-2xl border border-border/[0.06] bg-white/[0.02] p-12 text-center">
 <Sparkles className="h-8 w-8 text-muted mx-auto mb-3" />
 <h3 className="text-base font-semibold text-secondary">No Matching Candidates</h3>
