...
export default function AddRoleModal({
 ...
 <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col rounded-2xl border border-border bg-surface shadow-elevated overflow-hidden">
 {/* Header */}
 ...
 className="w-full rounded-xl border border-border bg-surface px-4 py-2.5 text-sm text-text-primary outline-none transition focus:border-blue-500"
 />
 </div>
 ...
 <div className="text-center py-4 border border-dashed border-border/80 rounded-xl text-xs text-muted">
 ...
 <div 
 key={index} 
 className="flex items-end gap-3 bg-surface/40 p-3 rounded-xl border border-border/60 relative"
 ...
 <div 
 className="w-full mt-2 rounded-lg border border-border bg-surface p-1.5 shadow-xl space-y-0.5 max-h-48 overflow-y-auto"
 onClick={(e) => e.stopPropagation()}
 >
 {OPERATIONS.map((op) => {
 ...
 <label 
 key={op.value}
 className={`flex items-center gap-2.5 px-3 py-2 rounded-md cursor-pointer text-sm transition-colors ${isChecked ? 'bg-blue-600/20 text-white' : 'text-secondary hover:bg-secondary-surface hover:text-text-primary'}`}
 >
 <input
 type="checkbox"
 checked={isChecked}
 ...
 <button
 type="submit"
 className="rounded-xl bg-primary px-6 py-2.5 text-sm font-medium text-white transition hover:bg-primary-hover focus-ring"
 >
