...
export default function PermissionMatrix({ roles, onRefresh, loading }: PermissionMatrixProps) {
 ...
 return (
 <div className="overflow-hidden rounded-2xl border border-red-500/20 bg-background p-8 text-center shadow-lg">
 ...
 {/* Toast Notification */}
 {toast && (
 <div className={`absolute top-4 right-4 z-50 flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium transition-all ${
 toast.type === "success" 
 ...
 <th
 key={role.id}
 className="px-6 py-4 text-center text-sm font-medium text-muted"
 >
 ...
 >
 <td className="px-6 py-4 text-sm text-text-primary flex flex-col">
 <span>{pInfo.label}</span>
 <span className="text-[10px] text-muted font-mono mt-0.5">{pInfo.key}</span>
 </td>
 ...
 <input
 type="checkbox"
 className="h-4 w-4 rounded border-border bg-surface text-blue-600 focus:ring-blue-500 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
 checked={isChecked}
