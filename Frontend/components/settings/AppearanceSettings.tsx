...
function LightPreview() {
 return (
 <div className="h-20 w-full flex flex-col gap-1.5 p-2.5 bg-slate-50 border-b border-border">
...
function DarkPreview() {
 return (
 <div className="h-20 w-full flex flex-col gap-1.5 p-2.5 bg-background border-b border-border">
 <div className="h-2.5 w-3/5 rounded bg-slate-700" />
 <div className="h-2 w-4/5 rounded bg-secondary-surface" />
 <div className="h-2 w-2/5 rounded bg-secondary-surface" />
 <div className="mt-1 h-5 w-14 rounded-md bg-indigo-600" />
 </div>
 );
}
...
function SystemPreview() {
 return (
 <div className="h-20 w-full flex border-b border-border">
 <div className="flex-1 flex flex-col gap-1.5 p-2 bg-slate-50">
 <div className="h-2 w-full rounded bg-slate-300" />
 <div className="h-2 w-3/4 rounded bg-slate-200" />
 </div>
 <div className="flex-1 flex flex-col gap-1.5 p-2 bg-background">
 <div className="h-2 w-full rounded bg-slate-700" />
 <div className="h-2 w-3/4 rounded bg-secondary-surface" />
 </div>
 </div>
 );
}
...
function ThemeCard({ label, description, icon, selected, onSelect, preview }: ThemeCardProps) {
 ...
 >
 {preview}
 <div className="flex items-start justify-between gap-2 p-3 w-full bg-surface border-t border-border">
 <div className="flex items-center gap-2">
 <span className={selected ? "text-indigo-500" : "text-muted"}>{icon}</span>
 <div>
 <p className={`text-[12.5px] font-semibold ${selected ? "text-indigo-500" : "text-primary"}`}>
 {label}
 </p>
...
function ToggleOption({ icon, title, description, checked, onChange }: ToggleOptionProps) {
 ...
 : "bg-slate-850 border border-border text-muted"
 }`}
 >
 {icon}
 </span>
 ...
 checked ? "bg-indigo-600" : "bg-secondary-surface"
 }`}
 >
 <span
...
export default function AppearanceSettings() {
 ...
 return (
 <div className="rounded-2xl overflow-hidden shadow-lg bg-background border border-border">
 {/* Header */}
 ...
 {/* Footer */}
 <div className="flex items-center justify-between gap-3 px-7 py-4 border-t border-border bg-surface">
 <p className="text-[11.5px] text-muted">
 Theme is saved to your account across devices.
 </p>
 <button
 onClick={handleSave}
 className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-[12.5px] font-semibold text-white shadow-sm transition-all bg-indigo-600 hover:bg-indigo-500 active:scale-95 active:bg-indigo-700"
 >
 {saved
