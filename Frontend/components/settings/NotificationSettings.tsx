...
function ToggleRow({ icon, title, description, checked, onChange }: ToggleRowProps) {
 ...
 checked ? "bg-indigo-600/10 border-indigo-600/30" : "bg-background border-border"
 }`}>
 <div className="flex items-start gap-3">
 <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors border ${
 checked ? "bg-indigo-600 border-indigo-600/20 text-white" : "bg-secondary-surface border-border text-muted"
 }`}>
 {icon}
 </span>
 <div>
...
export default function NotificationSettings() {
 ...
 {/* Header */}
 <div className="px-7 py-5 flex items-start justify-between border-b border-border">
 <div>
 ...
 : "bg-background border-border text-muted hover:text-muted"
 }`}
 >
 ...
 <p className={`text-[11.5px] ${saveError ? "text-red-400" : "text-muted"}`}>
 {saveError || "Preferences saved per device."}
 </p>
 <button
 onClick={handleSave}
 disabled={loading}
 className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-[12.5px] font-semibold text-white shadow-sm transition-all active:scale-95 active:bg-indigo-700 disabled:opacity-50 bg-indigo-600 hover:bg-indigo-500 keep-white"
 >
 {saved
