...
export default function SettingsSidebar({ activeTab, setActiveTab }: SettingsSidebarProps) {
 ...
 {/* Nav card */}
 <div className="rounded-2xl p-2 bg-surface border border-border">
 ...
 : "text-muted hover:bg-slate-850 hover:text-secondary"
 }`}
 >
 {/* Icon */}
 <span
 ...
 isActive
 ? "bg-secondary-surface text-text-primary"
 : "bg-secondary-surface text-muted group-hover:bg-slate-750 group-hover:text-muted"
 }`}
 >
 ...
 isActive ? "text-text-primary" : "text-secondary group-hover:text-primary"
 }`}>
 {item.label}
 </div>
 <div className={`mt-0.5 text-[11px] truncate ${
 isActive ? "text-secondary" : "text-muted group-hover:text-muted"
 }`}>
 {item.description}
 </div>
 </div>
 ...
 isActive ? "text-text-secondary" : "text-transparent group-hover:text-muted"
 }`}
 />
 </button>
 );
