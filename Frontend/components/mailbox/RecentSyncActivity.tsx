...
function SyncCard({ item, index }: { item: SyncHistoryItem; index: number }) {
 ...
 className="flex gap-3 rounded-xl border border-border bg-secondary-surface p-3.5 transition hover:bg-secondary-surface hover:border-border"
 >
 ...
 <span className="flex items-center gap-1 rounded-md border border-border bg-secondary-surface px-2 py-0.5 text-[11px] font-medium text-text-secondary">
 ...
 <span className="rounded-md border border-border bg-secondary-surface px-2 py-0.5 text-[11px] font-medium text-gray-500">
 ⏱ {item.duration_seconds}s
 </span>
 )}

 {item.status === "Completed" && item.emails_processed === 0 && (
 <span className="rounded-md border border-border bg-secondary-surface px-2 py-0.5 text-[11px] font-medium text-gray-600">
 No new emails
 </span>
 )}
 </div>
...
function ViewAllModal({ history, total, onClose }: {
 ...
 <div>
 <h2 className="text-2xl font-bold text-text-primary">All Sync Activity</h2>
 <p className="mt-1 text-sm text-muted">{total} total sync{total !== 1 ? "s" : ""}</p>
 </div>
 <button
 onClick={onClose}
...
export default function RecentSyncActivity() {
 ...
 <div className="flex items-center justify-between border-b border-border px-6 py-5">
 <div>
 <h2 className="text-xl font-semibold text-text-primary">Recent Sync Activity</h2>
 ...
 title="Refresh"
 disabled={loading}
 className="rounded-lg p-2 text-text-secondary hover:bg-secondary-surface hover:text-text-primary transition disabled:opacity-40"
 >
 ...
 <InboxIcon className="h-8 w-8 opacity-30" />
 <span className="font-medium text-text-secondary">No sync history yet.</span>
