...
const STAGE_META: Record<string, { icon: React.ReactNode; color: string }> = {
 Applied: { icon: <ClipboardCheck className="h-3.5 w-3.5" />, color: "text-primary" },
...
const STAGE_CONFIG: Record<string, ConfigFactory> = {
 ...
 primary = null;
 secondary = [
 can(props.userRole, "view_offer") && { label: "View Offer", icon: <FileText className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => { props.onViewOffer?.(id, offerId ?? undefined); close(); } },
 can(props.userRole, "edit_offer") && { label: "Edit Offer", icon: <Pencil className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => { props.onEditOffer?.(id, offerId ?? undefined); close(); } },
 can(props.userRole, "view_profile") && { label: "View Profile", icon: <Eye className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => call(props.onViewProfile) },
 can(props.userRole, "add_note") && { label: "Add Note", icon: <MessageSquarePlus className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => call(props.onAddNote) },
 can(props.userRole, "view_timeline") && { label: "View Timeline", icon: <History className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => call(props.onViewTimeline) },
 can(props.userRole, "remove_candidate") && { label: "Remove Candidate", icon: <XCircle className="h-3.5 w-3.5" />, colorClass: "text-red-400", bgHover: "hover:bg-red-500/10", onClick: () => setConfirm?.("remove") },
...
const DEFAULT_CONFIG: ConfigFactory = (id, props, call, offerId, setConfirm) => ({
 ...
 bgClass: "bg-secondary-surface",
 borderClass: "border-border",
 onClick: () => { },
 },
 secondary: [
 { label: "View Profile", icon: <Eye className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => call(props.onViewProfile) },
 { label: "View Timeline", icon: <History className="h-3.5 w-3.5" />, colorClass: "text-text-secondary", bgHover: "hover:bg-secondary-surface", onClick: () => call(props.onViewTimeline) },
 ],
 showReject: false,
});
...
function ConfirmDialog({ message, confirmLabel, confirmColor, onConfirm, onCancel }: ConfirmDialogProps) {
 ...
 className="flex-1 rounded-lg border border-border bg-secondary-surface px-3 py-2 text-xs font-medium text-text-secondary transition-colors hover:bg-secondary-surface"
 >
 Cancel
 </button>
 <button
 ...
 onPointerDown={(e) => e.stopPropagation()}
 className={`flex-1 rounded-lg px-3 py-2 text-xs font-semibold text-text-primary transition-colors ${confirmColor}`}
 >
 {confirmLabel}
 </button>
 </div>
...
function DropdownPortal({
 ...
 className="fixed z-[9999] rounded-xl border border-border bg-surface p-2 shadow-2xl shadow-black/50 animate-in fade-in slide-in-from-top-1 duration-150"
 onPointerDown={(e) => e.stopPropagation()}
...
export default function CandidateQuickActions(props: CandidateQuickActionsProps) {
 ...
 const stageMeta = STAGE_META[stage] ?? { icon: <ArrowRight className="h-3.5 w-3.5" />, color: "text-text-secondary" };
 ...
 <div className="flex items-center gap-2 rounded-lg bg-secondary-surface px-3 py-2.5 text-xs text-gray-500">
